...
$(document).ready(function () {
    ...
    CargarDatos = function () {
        ...
        $.ajax({
            type: "POST",
            url: "EntradaSalidaZilmaxManejo.aspx/TraerEntradaSalidaZilmaxManejo",
            datatype: "Json",
            cache: false,
                        ...
                        if (data.d[i].Tipo == 'entrada') {
                            if (data.d[i].Estatus == 'ya debe entrar a zilmax') {
                                $("#tbEntradaZilmax tbody").append("<tr>" +
                                  ...
                                  "<td style='background-color:red; text-align: center;'>&nbsp;</td>" +
                                  "<td style='background-color:red; text-align: center;'><input type='checkbox' class='chkEntrada' id=" + data.d[i].LoteID + " /></td>" +
                                  "</tr>");
                            }
                            else {
                                $("#tbEntradaZilmax tbody").append("<tr>" +
                                 ...
                                 "<td style='text-align: center;'>&nbsp;</td>" +
                                 "<td style='text-align: center;'><input type='checkbox' class='chkEntrada' id=" + data.d[i].LoteID + " /></td>" +
                                 "</tr>");
                            }
                        }
                        else {
                            if (data.d[i].Tipo == 'salida') {
                                ...
                                if (data.d[i].Estatus == 'por salida a zilmax pero no se les a servido formula de retiro') {
                                    $("#tbSalidaZilmax tbody").append("<tr>" +
                                      "<td style='background-color:red;text-align: center;'>&nbsp;</td>" +
                                      "<td style='background-color:red;text-align: center;'>&nbsp;</td>" +
                                      "<td style='background-color:red; text-align: center;'><input type='checkbox' class='chkSalida' id=" + data.d[i].LoteID + " /></td>" +
                                      "</tr>");
                                }
                                else {
                                    $("#tbSalidaZilmax tbody").append("<tr>" +
                                      "<td style='text-align: center;'>&nbsp;</td>" +
                                      "<td style='text-align: center;'>&nbsp;</td>" +
                                      "<td style='text-align: center;'><input type='checkbox' class='chkSalida' id=" + data.d[i].LoteID + " /></td>" +
                                      "</tr>");
                                }
                            }
                        }
...
guardar = function () {
    ...
    $("input:checked").each(function () {
        var lote = {};
        lote.LoteID = $(this).attr("id");
        
        if ($(this).attr("class") == "chkSalida") {
            lote.FechaSalidaZilmax = ToDate($("#datepicker").val());
        } else {
            lote.FechaEntradaZilmax = ToDate($("#datepicker").val());
        }

        lotes.push(lote);
    });
    if (lotes.length > 0) {
        var valores = { "lotesZilmax": lotes };
        App.bloquearContenedor($(".container-fluid"));
        $.ajax({
            type: "POST",
            url: "EntradaSalidaZilmaxManejo.aspx/GuardarEntradaSalidaZilmaxManejo",
            datatype: "Json",
            cache: false,
            contentType: "application/json; charset=utf-8",
                    ...
                    bootbox.dialog({
                        ...
                        buttons: {
                            Aceptar: {
                                label: "Ok",
                                callback: function () {
                                    window.location.href = 'EntradaSalidaZilmaxManejo.aspx';
                                }
                            },
                        }
                    });                    
