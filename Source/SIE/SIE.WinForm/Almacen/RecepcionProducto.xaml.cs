                                ...
                                if (txtImporte.Value != null)
                                {
                                    if (txtImporte.Value != 0)
                                    {
                                        if (renglon < 0)
                                        {
                                            ...
                                            recepcionProductoDetalle.Importe = (decimal) txtImporte.Value;
            ...
            try
            {
                if (!string.IsNullOrEmpty(txtOrdenCompra.Text))
                {
                    if (recepcionProducto.ListaRecepcionProductoDetalle != null)
                    {
                        ...
                        {
                            if (recepcionProductoCompra.ListaRecepcionProductoDetalle.Count ==
                                recepcionProducto.ListaRecepcionProductoDetalle.Count)
                            {
                                if (recepcionProducto.ListaRecepcionProductoDetalle.Sum(registro => registro.Importe) ==
                                    recepcionProductoCompra.ListaRecepcionProductoDetalle.Sum(
                                        registro => registro.Importe))
                                {
                                    ...
                                    if (retorno != null)
                                    {
                                        ...
                                        exportarPoliza.ImprimirPoliza(retorno,
                                                                      string.Format("Poliza de Entrada de Orden de Compra Folio No. {0}",
                                                                                    recepcionProducto.FolioOrdenCompra));
                                        LimpiarTodo();
                                    }
                                    else
                                    {
                                ...
                                {
                                    SkMessageBox.Show(Application.Current.Windows[ConstantesVista.WindowPrincipal],
                                        Properties.Resources.RecepcionProducto_msgImportesDiferentes,
                                        MessageBoxButton.OK,
                                        MessageImage.Stop);
                                }
                            }
                            ...
                            {
                                SkMessageBox.Show(Application.Current.Windows[ConstantesVista.WindowPrincipal],
                                        Properties.Resources.RecepcionProducto_CantidadDiferenteGuardar,
                                        MessageBoxButton.OK,
                                        MessageImage.Stop);
                            }
                        }
            ...
            if (txtOrdenCompra.Text != "")
            {
                try
                {
                    if (!string.IsNullOrEmpty(servidor) && !string.IsNullOrEmpty(baseDatos))
                    {
                        ...
                        {
                            var recepcionExistente = recepcionProductoPl.ObtenerRecepcionPorFolio(recepcionProductoCompra);
                            ...
                            else
                            {
                                ...
                                dgRecepcion.ItemsSource = new List<RecepcionProductoDetalleInfo>();
                                SkMessageBox.Show(Application.Current.Windows[ConstantesVista.WindowPrincipal],
                                Properties.Resources.RecepcionProducto_msgOrdenRecibida,
                                MessageBoxButton.OK,
                                MessageImage.Stop);
                                txtOrdenCompra.Focus();
                            }
